...
import { Calculator, ShieldCheck, Eye, EyeOff, AlertCircle } from 'lucide-react';

const OAUTH_ERROR_MESSAGES: Record<string, string> = {
  invalid_oauth_state: 'Google sign-in session expired or was invalid. Please try signing in again.',
  oauth_token_exchange_failed: 'Google authorization could not be completed. Please try again.',
  oauth_userinfo_failed: 'Unable to retrieve your Google profile. Please try again.',
  oauth_email_missing: 'Your Google profile did not provide an email address. Please sign in with email.',
  oauth_unconfigured: 'Google sign-in is not configured on this LifeCalc deployment.',
  oauth_internal_error: 'Google sign-in encountered an internal error. Please try again or sign in with email.',
};
...
function SignInForm() {
  ...
  const oauthErrorMessage = oauthErrorCode
    ? OAUTH_ERROR_MESSAGES[oauthErrorCode] || 'Sign-in could not be completed. Please try again.'
    : null;
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-5">
      {activeError && (
        <div
          ...
          className="p-3.5 bg-rose-50 border border-rose-200 text-rose-700 text-xs rounded-xl flex items-start gap-2.5"
        >
